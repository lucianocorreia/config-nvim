  ...
  local plugin_maps = {
    {
      ...
      { 'v', '<leader>cf', function()
        ...
        end, '[F]ormat selected text' },
    },
    {
      { 'i', '<C-g>', function() require('blink.cmp').show { providers = { 'copilot' } } end, 'Mostrar sugestões do Copilot' },
      { 'i', '<Tab>', function()
        ...
        end, 'Copilot: Aceita sugestão ou Tab normal' },
    },
    {
    ...
    {
      { 'n', ']t', function() require('todo-comments').jump_next() end, 'Next todo comment' },
      { 'n', '[t', function() require('todo-comments').jump_prev() end, 'Previous todo comment' },
    },
    {
      { 'n', '<leader>ql', function() require('quicker').toggle() end, 'Toggle quickfix' },
    },
    {
    ...
    {
      { { 'n', 't', 'i', 'x' }, '<leader>aa', function() require('sidekick.cli').toggle() end, 'Sidekick: Toggle CLI' },
      { 'n', '<leader>as', function() require('sidekick.cli').select { filter = { installed = true } } end, 'Sidekick: Select CLI Tool' },
      { 'n', '<leader>ad', function() require('sidekick.cli').close() end, 'Sidekick: Detach CLI Session' },
      { { 'n', 'x' }, '<leader>at', function() require('sidekick.cli').send { msg = '{this}' } end, 'Sidekick: Send This' },
    ...
    {
      { 'n', '<leader>ff', snacks_call(function(Snacks)
          Snacks.picker.smart {
      ...
      { 'n', '<leader><leader>', snacks_call(function(Snacks) Snacks.picker.buffers { layout = { preview = { enabled = false } } } end), 'Buffers' },
      { 'n', '<leader>:', snacks_picker('command_history'), 'Command History' },
      { 'n', '<leader>fc', snacks_call(function(Snacks) Snacks.picker.files { cwd = vim.fn.stdpath 'config' } end), 'Find Config File' },
      { 'n', '<leader>fg', snacks_call(function(Snacks)
      ...
      { 'n', '<leader>sf', snacks_call(function(Snacks)
          Snacks.picker.files {
            layout = { preview = { enabled = false } },
            sort = function(a, b)
              if a.score ~= b.score then
      ...
      { 'n', '<leader>fr', snacks_call(function(Snacks) Snacks.picker.recent { filter = { cwd = true } } end), 'Recent files' },
      ...
      { 'n', '<leader>sd', snacks_call(function(Snacks) Snacks.picker.diagnostics_buffer { layout = { preview = { enabled = false } } } end), 'Diagnostics' },
      { 'n', '<leader>sD', snacks_call(function(Snacks) Snacks.picker.diagnostics { layout = { preview = { enabled = false } } } end), 'Buffer Diagnostics' },
      ...
      { 'n', '<leader>sk', snacks_call(function(Snacks) Snacks.picker.keymaps { layout = { preview = { enabled = false } } } end), 'Keymaps' },
      ...
      { 'n', 'gro', snacks_call(function(Snacks)
      ...
      { 'n', '<leader>n', snacks_call(function(Snacks) Snacks.notifier.show_history() end), 'Notification History' },
      { 'n', '<leader>bd', snacks_call(function(Snacks) Snacks.bufdelete() end), 'Delete Buffer' },
      { 'n', '<leader>cR', snacks_call(function(Snacks) Snacks.rename.rename_file() end), 'Rename File' },
      { { 'n', 'v' }, '<leader>gB', snacks_call(function(Snacks) Snacks.gitbrowse() end), 'Git Browse' },
      { 'n', '<leader>gg', snacks_call(function(Snacks) Snacks.lazygit() end), 'Lazygit' },
      { 'n', '<leader>un', snacks_call(function(Snacks) Snacks.notifier.hide() end), 'Dismiss All Notifications' },
      { 'n', '<C-/>', snacks_call(function(Snacks) Snacks.terminal() end), 'Toggle Terminal' },
      { 't', '<C-/>', snacks_call(function(Snacks) Snacks.terminal() end), 'Toggle Terminal' },
      { 'n', '<C-_>', snacks_call(function(Snacks) Snacks.terminal() end), 'which_key_ignore' },
      { { 'n', 't' }, ']]', function() require('snacks').words.jump(vim.v.count1) end, 'Next Reference' },
      { { 'n', 't' }, '[[', function() require('snacks').words.jump(-vim.v.count1) end, 'Prev Reference' },
